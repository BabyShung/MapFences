...
@interface GeoFencesOverlayRenderer ()

@property (strong, nonatomic) GeoFencesOverlay *selectorOverlay;

@end

@implementation GeoFencesOverlayRenderer
...
//    CGRect overlayRect = [self rectForMapRect:_selectorOverlay.boundingMapRect];
    ...
    if (!_selectorOverlay.fillInside) {
    ...
    CGFloat kDashedLinesLength[] = {overlayRect.size.width * .03f, overlayRect.size.width * .03f};
    CGContextSetLineWidth(context, overlayRect.size.width *.01f);
    ...
    CGContextAddArc(context, overlayRect.origin.x + radiusAtLatitude, overlayRect.origin.y, radiusAtLatitude * (_selectorOverlay.editingRadius ? .095f : .015f), 0, 2 * M_PI, true);
    CGContextDrawPath(context, kCGPathFillStroke);
