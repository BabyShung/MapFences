...
#import "GeoFencesOverlay.h"
#import "GeoFencesOverlayRenderer.h"

#define DEFAULT_RADIUS 1000
#define DEFAULT_MIN_Distance 100
#define DEFAULT_MAX_Distance 10000

@interface GeoFencesViewController ()
{
    GeoFencesOverlay            *_selectorOverlay;
    GeoFencesOverlayRenderer    *_selectorOverlayRenderer;
...
- (void)setupDefaults
{
    ...
    self.circleRadiusMin = DEFAULT_MIN_Distance;
    self.circleRadiusMax = DEFAULT_MAX_Distance;
    self.hidden = NO;
    self.fillInside = YES;
}
...
- (void)viewDidLoad
{
    ...
    //add overlay: specify coordinate and bounding rect
    _selectorOverlay = [[GeoFencesOverlay alloc] initWithCenterCoordinate:_circleCoordinate radius:_circleRadius];
...
- (GeoFencesGestureRecognizer *)selectorGestureRecognizer
{
    
    __weak typeof(self)weakSelf = self;
    GeoFencesGestureRecognizer *selectorGestureRecognizer = [[GeoFencesGestureRecognizer alloc] init];
...
- (void)setCircleCoordinate:(CLLocationCoordinate2D)circleCoordinate
{
    if ((_circleCoordinate.latitude != circleCoordinate.latitude) || (_circleCoordinate.longitude != circleCoordinate.longitude)) {
        ...
        for (id<MKAnnotation> currentAnnotation in self.mapView.annotations) {
            if ([currentAnnotation isKindOfClass:GeoFencesAnnotation.class]) {
...
- (void)displaySelectorAnnotationIfNeeded
{
    for (id<MKAnnotation> annotation in self.mapView.annotations) {
        if ([annotation isKindOfClass:[GeoFencesAnnotation class]]) {
            [self.mapView removeAnnotation:annotation];
        }
    }
    
    if (_hidden == NO && ((_editingType == DBMapSelectorEditingTypeFull) || (_editingType == DBMapSelectorEditingTypeCoordinateOnly))) {
        GeoFencesAnnotation *selectorAnnotation = [[GeoFencesAnnotation alloc] init];
...
{
    if ([annotationView.annotation isKindOfClass:[GeoFencesAnnotation class]]) {
...
- (MKOverlayRenderer *)mapView:(MKMapView *)mapView rendererForOverlay:(id<MKOverlay>)overlay
{
    ...
    if ([overlay isKindOfClass:[GeoFencesOverlay class]]) {
        _selectorOverlayRenderer = [[GeoFencesOverlayRenderer alloc] initWithSelectorOverlay:(GeoFencesOverlay *)overlay];
